...
class LoadMQO(Model3D):
    ...
    def add_material(self,lines,index):
        ...
        glGenTextures(num, ctypes.byref(textureIDs))
        """
    ...
    def add_vertex(self, lines, index, mesh):
        ...
        y = []
        z = []
        self.landmark_nomalize = np.empty((0, 3))
        for i in range(index+1, num+index+1):
            line = lines[i].split(' ')
            ...
            self.landmark_nomalize = np.append(self.landmark_nomalize, np.array([[float(line[0]),
                                                                        float(line[1]),
                                                                        float(line[2])]]), axis=0)
